...
- [ ] **Brevo account setup** (plan, contacts, domain auth) - runbook written 2026-07-30 in
      **`project/BREVO-SETUP.md`**. Needs Brevo account access + the client's web/IT for DNS.
...
- [ ] **Brevo SPF/DKIM for kelvincave.com** - needs the client's web/IT. Longest-lead item for a
      2,500-contact send, so request it now rather than on send day. **kelvincave.com already has
      an SPF record ending in `-all`, at 7 of the 10 allowed DNS lookups - it must be EDITED to add
      `include:spf.brevo.com`, never duplicated.** Full detail in `BREVO-SETUP.md` section 3a.
...
**Account/plan/contacts/DNS setup is a separate doc: `project/BREVO-SETUP.md`.** Do that first -
the domain has to be authenticated before a 2,500-contact send is viable. This section is just the
campaign build.
